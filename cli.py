...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("evo_learn.log")],
)
...
def setup_argparse():
    ...
    train_parser.add_argument(
        "--generations", type=int, default=5, help="TPOT generations"
    )
    train_parser.add_argument(
        "--population", type=int, default=20, help="TPOT population size"
    )
    train_parser.add_argument(
        "--test-size", type=float, default=0.2, help="Test split size"
    )
    train_parser.add_argument(
        "--output-dir", default="models", help="Model output directory"
    )
    train_parser.add_argument(
        "--max-time", type=int, default=None, help="Total TPOT time (mins)"
    )
    train_parser.add_argument(
        "--max-eval-time", type=int, default=5, help="Per-pipeline eval time (mins)"
    )
    train_parser.add_argument(
    ...
    train_parser.add_argument(
        "--no-categoricals", action="store_true", help="Disable categorical encoding"
    )
    train_parser.add_argument(
        "--impute",
    ...
    train_parser.add_argument(
        "--no-scale", action="store_true", help="Disable numeric scaling"
    )
    # Baseline option
    train_parser.add_argument(
        "--baseline", action="store_true", help="Skip TPOT and train baseline model"
    )

    # Predict command
    predict_parser = subparsers.add_parser(
        "predict", help="Predict with a trained model"
    )
    predict_parser.add_argument("--model", required=True, help="Path to trained model")
    predict_parser.add_argument(
        "--data", required=True, help="Path to prediction data (CSV)"
    )
    predict_parser.add_argument(
        "--output", default="predictions.csv", help="Where to save predictions CSV"
    )
    ...
    eval_parser.add_argument(
        "--output-dir", default="evaluation", help="Directory for evaluation artifacts"
    )

    # Visualize command
    viz_parser = subparsers.add_parser(
        "visualize", help="Standalone data visualizations"
    )
    ...
    viz_parser.add_argument(
        "--output-dir", default="visualizations", help="Directory for plots"
    )
...
def train_model(args):
    ...
    overlay_path = None
    try:
        from core import _load_config, run_automl
...
def predict_with_model(args):
    ...
    try:
        from core import load_data, load_model, predict
...
def evaluate_model(args):
    ...
    try:
        from core import load_data, load_model
        ...
        if np.issubdtype(y.dtype, np.number):
            ...
            save_actual_vs_pred(
                y, y_pred, output_path / f"actual_vs_pred_{timestamp}.png"
            )
...
def create_visualizations(args):
    ...
    try:
        ...
        from visualization import plot_correlation_matrix, plot_feature_distributions
        ...
        plot_feature_distributions(
            data=data, target_column=args.target, output_dir=output_path
        )
        plot_correlation_matrix(
            data=data, output_path=output_path / "correlation_matrix.png"
        )
...
def show_version(args=None):
    ...
    print(
        "Features: Robust preprocessing, baseline fallbacks, comprehensive evaluation"
    )
