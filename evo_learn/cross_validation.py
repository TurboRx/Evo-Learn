...
def cross_validate_model(
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splitter.split(X, y)):
        ...
        fold_predictions.append(
            {"y_true": y_val_fold, "y_pred": y_pred, "indices": val_idx}
        )
    ...
    for outer_idx, (train_idx, test_idx) in enumerate(outer_splitter.split(X, y)):
        ...
        if task.lower() == "classification":
            scores = {
                ...
                "f1": f1_score(
                    y_test_outer, y_pred, average="weighted", zero_division=0
                ),
            }
        else:
            mse = mean_squared_error(y_test_outer, y_pred)
            scores = {"rmse": np.sqrt(mse), "r2": r2_score(y_test_outer, y_pred)}
