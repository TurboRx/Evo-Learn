...
class ModelExplainer:
    ...
    def compute_shap_values(
        self, X_test: pd.DataFrame, max_samples: int = 100, explainer_type: str = "auto"
    ) -> np.ndarray:
        ...
        if hasattr(self.model, "named_steps"):
            ...
            estimator_step = next(
                (
                    ...
                    if hasattr(step[1], "predict") or hasattr(step[1], "predict_proba")
                ),
                None,
            )
            if estimator_step is None:
                raise ValueError(
                    "Pipeline is missing an estimator step with predict/predict_proba."
                )
            ...
            if explainer_type == "auto":
                ...
                try:
                    ...
                    self.explainer = shap.KernelExplainer(
                        model_to_explain.predict, X_train_transformed
                    )
                ...
                self.explainer = shap.LinearExplainer(
                    model_to_explain, X_train_transformed
                )
            else:
    ...
    def plot_feature_importance(
        self, output_path: Optional[str] = None, max_display: int = 20
    ) -> None:
    ...
    def explain_prediction(
        self, sample_idx: int, output_path: Optional[str] = None
    ) -> None:
