...
class TestBuildPreprocessor:
    ...
    def test_imputation_strategy_mean(self):
        ...
        data = pd.DataFrame(
            {"feature1": [1.0, 2.0, np.nan, 4.0, 5.0], "target": [0, 1, 0, 1, 0]}
        )
    ...
    def test_categorical_imputation(self):
        ...
        data = pd.DataFrame(
            {"cat1": ["A", "B", None, "A", "A"], "target": [0, 1, 0, 1, 0]}
        )

        preprocessor, _ = build_preprocessor(data, "target", handle_categoricals=True)
    ...
    def test_handle_categoricals_false(self):
        ...
        preprocessor, _ = build_preprocessor(data, "target", handle_categoricals=False)
    ...
    def test_max_categorical_features_limit(self):
        ...
        data = pd.DataFrame(
            {"cat": [f"cat_{i}" for i in range(50)], "target": [0, 1] * 25}
        )
    ...
    def test_target_not_in_features(self):
        ...
        preprocessor, feature_names = build_preprocessor(data, "target")
    ...
    def test_sparse_output_false(self):
        ...
        data = pd.DataFrame(
            {"cat": ["A", "B", "C", "A", "B"], "target": [0, 1, 0, 1, 0]}
        )
    ...
    def test_unknown_categories_handling(self):
        ...
        data = pd.DataFrame(
            {"cat": ["A", "B", "A", "B", "A"], "target": [0, 1, 0, 1, 0]}
        )
    ...
    def test_all_nan_column_handling(self):
        ...
        preprocessor, _ = build_preprocessor(data, "target")
