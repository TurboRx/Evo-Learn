...
from core import load_data, run_automl, validate_data_for_training
...
"""
    ...
    def test_baseline_vs_tpot_fallback(self, tmp_path):
        ...
        data = pd.DataFrame(
            {"feature1": [1, 2, 3, 4, 5, 6], "target": [0, 1, 0, 1, 0, 1]}
        )
...
class TestBackwardCompatibility:
    ...
    def test_api_unchanged_for_basic_usage(self, tmp_path):
        ...
        data = pd.DataFrame({"x": np.random.randn(50), "y": [0, 1] * 25})

        data_path = tmp_path / "api_test.csv"
        data.to_csv(data_path, index=False)

        # Old-style API call should still work
        result = run_automl(data_path, "y", task="classification", always_baseline=True)
